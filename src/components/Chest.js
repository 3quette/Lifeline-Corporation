...
export class Chest extends Component {
 ...
 ondoctorClick(doctorid){
    ...
    this.state.doctors.map(doctor=>{
       ...
       <center> < button style={{border:"white"}} class="btn" value="Loin" type='submit' onClick={()=>{
           window.localStorage.setItem('doctorID',"Dr."+ doctor.firstname+" " +doctor.lastname);
           window.open("/Booking"); this.setState({showBooking:true})}}>Schedule Appointment</button></center>
        </div>);
       }
       return doctor;
    });
  ...
  <section class="home" id="home">
      ...
      <div>
      <div class="sp"><h1><u>Chest Specialist</u></h1></div>
      <div id="doctor">
      <ul>
        {this.state.doctors.map(doctor=>( 
        <button id='info_button_dr' onClick={()=>{this.ondoctorClick(doctor.id)}}><div><li id='doctors'>Doctor Name: Dr.{doctor.firstname}
